...
figure('OuterPosition', get(groot, 'Screensize'));
set(gcf, 'Renderer', 'opengl'); % Use OpenGL renderer for transparency
...
for radius = radii
    nexttile;
    i = 163; % Fixed index for the object
    closestpoint = reshape(result(3).closestpoint(i, :, :), [], 2);
    dist = result(3).dist(i, :);
    centreSeg = reshape(mean(result(3).seg(i, :, :), 2), [], 2); % Center of the segment
    ...
    % Draw transparent circular radius using `fill`
    theta = linspace(0, 2*pi, 100);
    x_circle = radius * cos(theta) + centreSeg(1);
    y_circle = radius * sin(theta) + centreSeg(2);
    fill(x_circle, y_circle, 'y', 'FaceAlpha', 0.25, 'EdgeColor', 'y', 'LineWidth', 1);

    % Mark the segment center
    plot(centreSeg(1), centreSeg(2), '.r');
    text(centreSeg(1), centreSeg(2), sprintf('%d', i), 'color', 'r', 'FontSize', 14, ...
        'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');

    % Draw arrows to closest points for detected risky buildings
    ...
    if isempty(ids)
        title_ = 'No risky-building detected';
    elseif length(ids) == 1
...
% Ensure OpenGL is used for proper transparency support
set(gcf, 'Renderer', 'opengl');
figure('OuterPosition', get(groot, 'Screensize')); % Create a full-screen figure
% radii = [0,83];
radii = [200, 400];
tiledlayout(length(radii), 3, 'TileIndexing', 'columnmajor'); % Create a tiled layout for plots
for l = 1:3
    algo = result(l).algo; % Algorithm name
    classSeg = result(l).class; % Segment classes
    segSeg = result(l).seg; % Segment coordinates
    centreSeg = reshape(mean(segSeg, 2), [], 2); % Calculate segment centers
    dist = result(l).dist; % Distance matrix
    distMax = max(dist(classSeg == 1, :), [], 'all'); % Maximum distance for class 1
    closestpoint = result(l).closestpoint; % Closest points for segments

    for t = 1:length(radii)
        nexttile;
        th = radii(t) / distMax; % Threshold based on radius and maximum distance
        classGT_ = zeros(length(classGT), 1);
        ...
        % Plot ground truth segments with appropriate colors
        for j = 1:length(classGT)
        ...
        % Plot detected segments and their thresholds
        for i = 1:length(classSeg)
            if classSeg(i) == 1
                radius = th * distMax;
                % Draw semi-transparent circles using 'fill'
                theta = linspace(0, 2 * pi, 100); % Create circle coordinates
                x_circle = centreSeg(i, 1) + radius * cos(theta);
                y_circle = centreSeg(i, 2) + radius * sin(theta);
                fill(x_circle, y_circle, 'y', 'FaceAlpha', 0.25, 'EdgeColor', 'y', 'LineWidth', 1);

                % Plot segment centers and quivers
        ...
        % Adjust axis properties and labels
        axis equal;
        axis tight;
        xlim_ = xlim;
...
% Save the figure
exportgraphics(gcf, 'Building-classes-2.png');
