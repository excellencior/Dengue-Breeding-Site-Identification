...
exportgraphics(gcf, 'Building-Classes-Single.png')
% exportgraphics(gcf,'building-classes.emf');


% Save predicted risky buildings for r = 83 in the specified format
radius = 83;
th = radius / distMax; % Threshold for normalized distance
risky_buildings = find(any(dist(classSeg == 1, :) / distMax <= th, 1)); % Indices of risky buildings

% Prepare data to save
output_file = 'risky_buildings_r83.txt'; % Output file name
fileID = fopen(output_file, 'w');
for i = 1:length(risky_buildings)
    % Get the index of the risky building
    idx = risky_buildings(i);
    
    % Get the class label for the building
    class_id = 1;
    
    % Get the coordinates of the 4 corners of the bounding box (segmentation polygon)
    corners = segGT(idx, :, :);
    corners = reshape(corners, [], 2); % Reshape to 2D array of (x, y)
...
disp(['Predicted risky buildings saved to ', output_file]);
