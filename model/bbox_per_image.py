...
def yolo_labels(mask_dir, save_dir):
    ...
    Mask file format: segmentation points separated by space, last value is the confidence score
    """
    all_bboxes = []
    for index, filename in enumerate(sort_files(os.listdir(mask_dir), get_coordinates_from_filename_buildings)):
        with open(os.path.join(mask_dir, filename), 'r') as f:
            ...
            for line in lines:
                ...
                seg_labels = np.array(points[:-1], dtype=np.float32)
                # seg_labels = seg_labels * 2048 # Rescale to original image size

                # Offset to the original orthophoto coordinates
                # seg_labels[0::2] += xpos
                # seg_labels[1::2] += ypos
                ...
                rbox = np.intp(rbox) # Rotated Bounding Box

                # Append the bounding box to the list
                all_bboxes.append(rbox.flatten())

                with open(label_file_path, 'a') as label_file:
                    label_file.write(f"{rbox[0][0]} {rbox[0][1]} {rbox[1][0]} {rbox[1][1]} {rbox[2][0]} {rbox[2][1]} {rbox[3][0]} {rbox[3][1]} {conf_score}\n")
